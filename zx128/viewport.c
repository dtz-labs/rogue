...
#define ZX_VIEWPORT_MAX_LEFT (NUMCOLS - ZX_MAP_COLS)
...
void zx_update_viewport(void)
{
    ...
    if (left == ZX_VIEWPORT_NONE)
        left = clamp_left(hero.x - ZX_MAP_COLS / 2);
    ...
    if (proom != NULL && !(proom->r_flags & ISGONE) && proom->r_max.x > 0)
    {
        ...
        if (proom->r_max.x <= ZX_MAP_COLS)
        {
            ...
            low = clamp_left(padded_right - ZX_MAP_COLS + 1);
            high = clamp_left(padded_left);
            if (low > high)
            {
                low = clamp_left(room_right - ZX_MAP_COLS + 1);
        ...
        max_left = clamp_left(room_right - ZX_MAP_COLS + 1);
    }
    ...
    while (hero.x - left > ZX_MAP_COLS - ZX_VIEWPORT_EDGE - 1 &&
           left < max_left)
    {
        left += ZX_VIEWPORT_STEP;
        if (left > max_left)
